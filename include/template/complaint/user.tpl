...
$province_id = $user_info['province_id']?$user_info['province_id']:$province_id;
              ...
              <tbody>							  
                <{foreach name=user from=$user_infos item=user_info}>				 
					...
					<a data-toggle="modal" href="#myModal"  title= "封停账号" ><i class="icon-pause" href="user.php?page_no=<{$page_no}>&method=pause&user_id=<{$user_info.user_id}>"></i></a>
					<{/if }>
					<{if $user_info.status == 0}>
					<a data-toggle="modal" href="#myModal" title= "解封账号" ><i class="icon-play" href="user.php?page_no=<{$page_no}>&method=play&user_id=<{$user_info.user_id}>"></i></a>
					<{/if }>
					&nbsp;
					<a data-toggle="modal" href="#myModal" title= "删除" ><i class="icon-remove" href="user.php?page_no=<{$page_no}>&method=del&user_id=<{$user_info.user_id}>" ></i></a>
					<{/if}>
					</td>
					</tr>
				<{/foreach}>
