...
$arr = array('start_date','end_date','province_id','buss_name','sp_name','sp_corp_code','complaint_type','question_type','complaint_level','buss_type','sp_code','month','wan','part_name');
$start_date = $end_date = $page_no = $province_id = $buss_name = $sp_name = $sp_corp_code = $complaint_type = $question_type = $complaint_level = $buss_type = $sp_code =$start_date = $end_date = $month = $wan = $part_name = "";
...
// if($start_date != '' && $end_date !=''){
	...
	if($_GET['download']==1)
	{
		$data['result'] = Complaint::customSingle($param,$start,0);
	}
	else 
	{
...
function array_to_string($data) {
	if(empty($data)||!isset($data['result'])||empty($data['result'])) {
		$dataStr="没有符合您要求的数据！^_^";
 		...
 		$dataStr = "业务名称\t月不规范定制件数\t环比增长量\t环比增长率\t申诉成功\t申诉失败\t未申诉量\t不规范定制/业务收入(百万)\t投诉类型\t业务线\t认定有效量\n ";
